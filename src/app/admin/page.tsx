...
export default function AdminPage() {
    ...
    const validateForm = (): string | null => {
        ...
        if (isNaN(Number(form.roll_no))) return 'Roll number must be a valid number';
        if (!form.name.trim()) return 'Name is required';
        if (!form.classvar.trim()) return 'Class is required';
        if (form.ph_no && isNaN(Number(form.ph_no))) return 'Phone number must be a valid number';
        return null;
    };
    ...
    return (
        <div className="min-h-screen bg-black text-white max-w-2xl mx-auto p-6">
